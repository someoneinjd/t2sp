...
function show_usage {
   echo "Options: (devcloud|local) (sgemm|dgemm|cgemm|zgemm|conv|capsule|pairhmm|qrd|trmm|syr2k) (a10|s10|gen9|gen12) (tiny|large) (hw|emulator) [bitstream]"
}
...
if [ "$2" != "sgemm" -a "$2" != "dgemm" -a "$2" != "cgemm" -a "$2" != "zgemm" -a "$2" != "conv"  -a  "$2" != "capsule" -a "$2" != "pairhmm" -a "$2" != "trmm" -a "$2" != "syr2k" ]; then
    show_usage
    return
else
    workload="$2"
