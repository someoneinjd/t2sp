...
int main()
{
    ...
    double alpha, beta;
    Halide::Runtime::Buffer<double> a(TOTAL_K, TOTAL_I), x(TOTAL_K), yy(TOTAL_I);
    ...
    Halide::Runtime::Buffer<double> y(III, II, I);
    gemv(a, x, y);
    ...
    double number_ops = 2 * (double)(III * II * I) * (double)(KKK * KK * K); // Total operations (GFLOP for GEMV), independent of designs
    double number_bytes = (double)(KKK * III) * (double)(KK * II) * (double)(K * I) * 4 +
                         (double)(KKK * KK * K) * 4 +
                         (double)(III * II * I) * 4;
    double exec_time= ExecTime();
    roofline(mem_bandwidth, compute_roof, number_ops, 2*number_bytes,exec_time);
