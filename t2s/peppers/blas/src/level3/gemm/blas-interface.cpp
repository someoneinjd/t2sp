// Our BLAS kernel's interface to users is MKL's BLAS interface
#include "mkl_blas.h"
...
// Headers for Halide/T2S types, data structures, and some utilities.
#include "Halide.h"
...
void GEMM(const char *transa, const char *transb, const MKL_INT *m, const MKL_INT *n, const MKL_INT *k,
    ...
    // Constants. They have been passed in with pointers because the interface is Fortran, which passes arguments in references.
    const MKL_INT M = *m;
    const MKL_INT N = *n;
    const MKL_INT K = *k;
    const MKL_INT LDA = *lda;
    const MKL_INT LDB = *ldb;
    const MKL_INT LDC = *ldc;

    // Check parameters according to BLAS interface conventions
    _halide_user_assert(M >= 0 && N >= 0 && K >= 0);
    _halide_user_assert(a != 0x0 && b != 0x0 && c != 0x0);
    _halide_user_assert(transa != BlasNoTrans || LDA >= std::max(1, M));
    _halide_user_assert(transa == BlasNoTrans || LDA >= std::max(1, K));
    _halide_user_assert(transb != BlasNoTrans || LDB >= std::max(1, K));
    _halide_user_assert(transb == BlasNoTrans || LDB >= std::max(1, N));
    _halide_user_assert(LDC >= std::max(1, M));
...
// Define the BLAS kernels in terms of our own gemm.
...
void CGEMM(const char *transa, const char *transb, const MKL_INT *m, const MKL_INT *n, const MKL_INT *k,
    ...
    GEMM<const void*, void, type_of_complex, 32>(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc);
}
...
void ZGEMM(const char *transa, const char *transb, const MKL_INT *m, const MKL_INT *n, const MKL_INT *k,
    ...
    GEMM<const void*, void, type_of_complex, 64>(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc);
}
